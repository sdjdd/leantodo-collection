...
Page({
  ...
  addTodo(todo) {
    ...
    for (let i = 0; i < todo.length; ++i) {
      if (this.data.todos.find(t => t.objectId === todo[i].objectId)) {
        continue
      }
      this.data.todos.unshift(todo[i])
      if (!todo[i].done) {
        this.data.activeCount++
      }
    }
    this.setData({ activeCount: this.data.activeCount, todos: this.data.todos })
  },
  removeTodo(objectId) {
    let delIdx = -1
    ...
    if (delIdx === -1) return
    this.data.todos.splice(delIdx, 1)
  ...
  updateTodo(objectId, attr) {
    let todo = this.data.todos.find(t => t.objectId === objectId)
